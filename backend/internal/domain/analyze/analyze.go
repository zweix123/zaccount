...
type CalculateAnalyzeDataResponse struct {
	Income    float64   // 收入
	Expense   float64   // 支出
...
// CalculateAnalyzeData 计算指定时间范围内的分析数据
// 遵循标准协议：funcName(ctx context.Context, req) (resp, err)
func CalculateAnalyzeData(ctx context.Context, req *CalculateAnalyzeDataRequest) (*CalculateAnalyzeDataResponse, error) {
	// 参数验证
	if req == nil {
	...
	return &CalculateAnalyzeDataResponse{
		Income:    income,
		Expense:   expense,
		Balance:   balance,
