...
func Close() error {
	...
	fmt.Println("update table success, row number: ", len(globalTransactionTable.Rows))
	globalTransactionTable = nil
	return nil
}

func loadTable(filePath string) ([]*Transaction, error) {
...
func updateTable(filePath string) error {
	...
	defer writer.Flush()
	writer.Write(Fields)
	for _, row := range globalTransactionTable.Rows {
